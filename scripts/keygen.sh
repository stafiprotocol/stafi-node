...
controller_pubkey=$(subkey inspect "${controller_mnemonic}" | grep -o ': .*' | sed '2!d' | tr -d ': ')
controller_address=$(subkey inspect "${controller_mnemonic}" | grep -o ': .*' | sed '3!d' | tr -d ': ')

new_grandpa_mnemonic=$(subkey generate | grep -o '`.*`' | tr -d '`')
grandpa_mnemonic=${1:-$new_grandpa_mnemonic}
grandpa_pubkey=$(subkey -e inspect "${grandpa_mnemonic}" | grep -o ': .*' | sed '2!d' | tr -d ': ')
grandpa_address=$(subkey -e inspect "${grandpa_mnemonic}" | grep -o ': .*' | sed '3!d' | tr -d ': ')

new_babe_mnemonic=$(subkey generate | grep -o '`.*`' | tr -d '`')
babe_mnemonic=${1:-$new_babe_mnemonic}
babe_pubkey=$(subkey inspect "${babe_mnemonic}" | grep -o ': .*' | sed '2!d' | tr -d ': ')
babe_address=$(subkey inspect "${babe_mnemonic}" | grep -o ': .*' | sed '3!d' | tr -d ': ')

new_imonline_mnemonic=$(subkey generate | grep -o '`.*`' | tr -d '`')
imonline_mnemonic=${1:-$new_imonline_mnemonic}
imonline_pubkey=$(subkey inspect "${imonline_mnemonic}" | grep -o ': .*' | sed '2!d' | tr -d ': ')
imonline_address=$(subkey inspect "${imonline_mnemonic}" | grep -o ': .*' | sed '3!d' | tr -d ': ')
