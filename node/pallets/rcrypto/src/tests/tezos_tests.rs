// Copyright 2018 Stafi Protocol, Inc.
// This file is part of Stafi.

// Stafi is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.

// Stafi is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.

// You should have received a copy of the GNU General Public License
// along with Stafi.  If not, see <http://www.gnu.org/licenses/>.

#[cfg(test)]
mod tezos_tests {
    // Note this useful idiom: importing names from outer (for mod tests) scope.a
    use super::super::super::tezos::sign::*;
    use hex_literal::*;

    #[test]
    fn test_tezos_sign() {
        let data = hex!("");
        let except_edsig = "edsigteQsDv3asrSke7Bf5p1uaqceSjni5LUhnDFBcqEBv8ZF1tdbQLwWAkxA61B5YMLqrFc6MQ8kBhfuYWch3gzQ5yfwgjWBbC";
        let except_sbytes = hex!("");

        let sk_with_prefix = "edskRiwkRKDGnXwxYgSNFx68vhLQ23RqVhYpVxjRmoUyKMjtEfeoREieDdGBk9meBPSMnYt5UnhxsgEyGd9EFm3mojgdVMkMBq";
        let sk = sk_with_prefix;
        let signer = Sign::default();
        let signature_data = signer.sign(data.to_vec(), sk);
        assert_eq!(signature_data.edsig, except_edsig.as_bytes());
        assert_eq!(signature_data.sbytes, except_sbytes.to_vec());
    }

    use super::super::super::tezos::generator::*;
    #[test]
    fn test_tezos_generate() {
        for _ in 0..3 {
            let keypair = generate_keypair();
            let (sk, pk, _pkh) = (keypair.sk, keypair.pk, keypair.pkh);
            let sk_str = str::from_utf8(&sk).unwrap();
            let pk_str = str::from_utf8(&pk).unwrap();
            let mnemonic_str = str::from_utf8(&keypair.mnemonic).unwrap();
            println!("sk: {}, pk: {}, mnemonic: {}", sk_str, pk_str, mnemonic_str);
        }
    }

    #[test]
    fn test_tezos_get_keypair_from_mnemonice() {
        let mnemonic_str = "burden mechanic acquire muscle flag home oil valley eager donor little essay daughter baby hover";
        let sk = "edskS2SDFgtTqWbEoKyW5CkXuwfki2NLzcGz6YDLQ5Pexp5iEuJgb8Wj6rG3D9pVrWRo9EJ4iihnqdvHx4cgSCGuTMjpSSSF7f";
        let pk = "edpkuw9X1bauTMKiAadWJJioLujYxADYrf4Q3dGRJbSmLbgGn3TC1j";
        let pkh = "tz1iJY251ptCJXWMdzhrkLNM72p2UZ48vuZb";
        let keypair = generate_keypair_from_mnemonic_str(mnemonic_str, "mWcziEO9fE8kzGsV");
        assert_eq!(sk.as_bytes(), keypair.sk.as_slice());
        assert_eq!(pk.as_bytes(), keypair.pk.as_slice());
        assert_eq!(pkh.as_bytes(), keypair.pkh.as_slice());
    }

    #[test]
    fn test_tezos_get_pubkey_from_mnemonice() {
        let test_cases = vec![("plug foster tuna tube strike gorilla island sense curious fantasy ranch jar shuffle type resist", "edpkvExwj1vy8kqKYzLQNFpbGcMLPTjZPnz9C22UbCJJYyvffn833j"), ("error believe card craft unit race foam pistol fresh ski help garden require soon include", "edpkuD8KKWjJNffff7BC9RQr79jpAbJAP8cfmQSEAA4ibr2TtaSNg3")];
        for (mnemonic_str, pk) in test_cases {
            let keypair = generate_keypair_from_mnemonic_str(mnemonic_str, "mWcziEO9fE8kzGsV");
            assert_eq!(pk.as_bytes(), keypair.pk.as_slice());
        }
    }

    use super::super::super::tezos::base58;
    use sp_std::str;
    #[test]
    fn test_tezos_pkh() {
        let pk = "edpkvQQhHzGoFf2zSESp1Kh57sFhbhtA16XAGXqjAYse75BC9RdoXW";
        let except_pkh = "tz1MNu6ytbdEYrHyyQwctJ7rZVFcLrHWjKoN";
        let raw_pk_with_prefix = base58::from_check(pk).unwrap();
        let pkh = pkh_from_rawpk(&raw_pk_with_prefix[4..]);
        assert_eq!(except_pkh.as_bytes().to_vec(), pkh);
    }

    use super::super::super::tezos::verify::*;
    #[test]
    fn test_tezos_verify() {
        let test_data = "TEST".as_bytes();
        let sk = "edskS2SDFgtTqWbEoKyW5CkXuwfki2NLzcGz6YDLQ5Pexp5iEuJgb8Wj6rG3D9pVrWRo9EJ4iihnqdvHx4cgSCGuTMjpSSSF7f";
        let pk = base58::from_check("edpkuw9X1bauTMKiAadWJJioLujYxADYrf4Q3dGRJbSmLbgGn3TC1j")
            .ok()
            .unwrap();
        let signer = Sign::default();
        let signature_data = signer.sign(test_data.to_vec(), sk);
        let (message, _) = signer.preprocess(test_data.to_vec());
        assert!(verify(&message, &signature_data.sig, &pk[4..]));
    }


    #[test]
    fn test_tezos_verify_with_ed() {
        let test_data = "op5bpKCL9Mo7bz2EGGsScpo3Ky1JzWAs4CmPGMAzNgRcBoyBkFk".as_bytes().to_vec();
        let edpk = "edpkvQQhHzGoFf2zSESp1Kh57sFhbhtA16XAGXqjAYse75BC9RdoXW".as_bytes().to_vec();
        let sk = "edskRsv7N9pK9tmpK9hPaLzGFACWY14aY27oCJxdeXyZ98ipfgknFMheswsWc88WZBmCWa1dfB3NyhUqfa8MWPvpiLK2UHKpA6";
        let signature_data = Sign::default().sign(test_data.clone(), sk);
        assert!(verify_with_ed(&test_data, &signature_data.edsig, &edpk, true));
    }
}
