...
pub fn keypair_from_raw_keypair(raw_sk: &[u8], raw_pk: &[u8]) -> (Vec<u8>, Vec<u8>, Vec<u8>) {
    // PubKey
...
mod tests {
    use super::*;
    #[test]
    fn test_pkh_from_rawpk() {
        let edpk_str = "edpkvExwj1vy8kqKYzLQNFpbGcMLPTjZPnz9C22UbCJJYyvffn833j";
        let raw_pk_with_prefix = base58::from_check(&edpk_str).unwrap();
        let pkh = pkh_from_rawpk(&raw_pk_with_prefix[4..]);
        assert_eq!("tz1TFXChtxVuJpc3ezZmky4S5YNPNEBEBMp9".as_bytes().to_vec(), pkh);
    }
}
