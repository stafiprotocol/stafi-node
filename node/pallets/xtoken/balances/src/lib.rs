...
decl_event!(
		...
		<T as frame_system::Trait>::AccountId,
	{
        ...
        Transfer(AccountId, AccountId, XSymbol, u128),
		/// Some balance was deposited
		Minted(AccountId, XSymbol, u128),
		/// Some balance was burned
		Burned(AccountId, XSymbol, u128),
	}
);
...
decl_storage! {
	trait Store for Module<T: Trait> as XBalances {
		/// The total units issued in the system.
        pub TotalIssuance get(fn total_issuance): map hasher(blake2_128_concat) XSymbol => u128;

		/// NOTE: This is only used in the case that this module is used to store balances.
		pub Account get(fn account):
			double_map hasher(blake2_128_concat) XSymbol, hasher(blake2_128_concat) T::AccountId => Option<AccountXData>;
	}
}
...
impl<T: Trait> Module<T> {
    pub fn mutate_account<R>(
		who: &T::AccountId,
		symbol: XSymbol,
		f: impl FnOnce(&mut AccountXData) -> R
	) -> R {
	...
	fn try_mutate_account<R, E>(
		who: &T::AccountId,
		symbol: XSymbol,
		f: impl FnOnce(&mut AccountXData) -> Result<R, E>
	) -> Result<R, E> {
...
{
	fn free_balance(who: &T::AccountId, symbol: XSymbol) -> u128 {
